...
- **Outlined icon-button: decided against.** Earlier this proposed a shared `.icon-button--outlined`. Cut from the ideal: the product has none, Material 22 has no native one, and it read almost identically to a plain icon button. Rule instead: icon buttons are plain (two sizes); an icon action that must read as a standalone button gets a text label.
